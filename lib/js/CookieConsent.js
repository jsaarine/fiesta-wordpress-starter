class CookieConsent {
	...
	build() {
		...
		this.el.querySelector(".c-button.accept").addEventListener("click", e => {
			this.createCookie(this.cookieName, JSON.stringify({necessary: 1, functional: 1, analytics: 1, marketing: 1}), 365);
			this.close();
		...
		if(this.declineButtonText) {
			this.el.querySelector(".c-button.decline").addEventListener("click", e => {
				this.createCookie(this.cookieName, JSON.stringify({necessary: 1}), 365);
				this.close();
			});
		}
