...
class Header {
  ...
  update() {
    const top = Core.scrollTop(); // Small (header is visible but smaller when scrolling)
...
class Navigation {
  ...
  build() {
    ...
    this.setupToggleNavigation(); // Adds clickable overlay
    // this.setupOverlay();
    // Adds double tab
  ...
  open() {
    ...
    this.toggleButton.setAttribute("aria-expanded", true); // Inert WIP
  ...
  close() {
    ...
    document.documentElement.classList.add("m-nav-open-transition");
    this.toggleButton.setAttribute("aria-expanded", false); // Inert WIP
    // if(this.el.parentNode == document.body) {
    // 	[].slice.call(document.body.children).forEach(item => {
    // 		item.inert = false;
    // 	});
  ...
  setupOverlay() {
    document.querySelectorAll("main-nav > ul > li.m-has-overlay > a").forEach(function (i) {
      ...
      i.addEventListener("click", function (e) {
        ...
        e.preventDefault();
        e.stopPropagation(); // Clear all opened navs
